...
public class EsfQtzhiKuaDuValidator extends EsfValidator {
	...
	private int getCount(String item)
	{
		...
		super.checkMargin(intArray, EsfConstants.QTZU_KUADU_MAX, EsfConstants.QTZU_KUADU_MIN);
		int count = 0;
		for(int i = 0; i < intArray.length; i++)
		{
			count += 2*EsfConstants.QTZU_KUADU_COUNT[intArray[i]];
		}
		return count;
	}
}
