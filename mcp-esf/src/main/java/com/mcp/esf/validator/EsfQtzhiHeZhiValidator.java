...
public class EsfQtzhiHeZhiValidator extends EsfValidator {
	...
	private int getCount(String item)
	{
		...
		super.checkMargin(intArray, EsfConstants.QTZU_HEZHI_MAX, EsfConstants.QTZU_HEZHI_MIN);
		int count = 0;
		for(int i = 0; i < intArray.length; i++)
		{
			count += 2*EsfConstants.QTZU_HEZHI_COUNT[intArray[i]];
		}
		return count;
	}
}
