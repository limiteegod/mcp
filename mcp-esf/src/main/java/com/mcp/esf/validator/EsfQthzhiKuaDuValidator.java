...
public class EsfQthzhiKuaDuValidator extends EsfValidator {
	...
	private int getCount(String item)
	{
		...
		super.checkMargin(intArray, EsfConstants.QTHZU_KUADU_MAX, EsfConstants.QTHZU_KUADU_MIN);
		int count = 0;
		for(int i = 0; i < intArray.length; i++)
		{
			count += 6*EsfConstants.QTHZU_KUADU_COUNT[intArray[i]];
		}
		return count;
	}
}
