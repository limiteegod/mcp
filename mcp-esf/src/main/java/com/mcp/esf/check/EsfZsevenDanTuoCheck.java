...
public class EsfZsevenDanTuoCheck extends EsfCheck {
	
	private static Logger log = Logger.getLogger(EsfZsevenDanTuoCheck.class);
	...
	public CheckParam check(TTicket ticket, String[] number, PrizeDescription prizeDescription)
		...
		if(danHit + tuoHit == 5 && tuoHit <= 7 - fCount)
		{
			int count = MathUtil.getC(sCount - tuoHit, 7 - fCount - tuoHit);
			if(count > 0)
			{
