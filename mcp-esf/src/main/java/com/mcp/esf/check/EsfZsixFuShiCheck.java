...
public class EsfZsixFuShiCheck extends EsfCheck {
	
	private static Logger log = Logger.getLogger(EsfZsixFuShiCheck.class);
	...
	public CheckParam check(TTicket ticket, String[] number, PrizeDescription prizeDescription)
		...
		if(hitCount == 5)
		{
			int count = MathUtil.getC(ticketNumberArray.length - 5, 1);
			if(count > 0)
			{
