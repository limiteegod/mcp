...
public class EsfZfiveDanTuoCheck extends EsfCheck {
	
	private static Logger log = Logger.getLogger(EsfZfiveDanTuoCheck.class);
	...
	public CheckParam check(TTicket ticket, String[] number, PrizeDescription prizeDescription)
		...
		if(getHitCount(fCountArray, number) == fCount)
		{
			...
			int count = MathUtil.getC(tuoHit, 5 - fCount);
			if(count > 0)
			{
