...
public class PlsZuSanKuaDuValidator extends PlsValidator {
	...
	public int validator(String numbers) throws CoreException {
		...
		if(!numbers.matches("^\\d{1}(,\\d{1}){0,8}$"))
		{
		...
		super.checkMargin(detailNumberArray, PlsConstants.ZUSAN_KUADU_MAX, PlsConstants.ZUSAN_KUADU_MIN);
		int count = 0;
		for(int j = 0; j < detailNumberArray.length; j++)
		{
			count += PlsConstants.ZUSAN_KUADU_COUNT[detailNumberArray[j]];
		}
		return count;
	}
