 ...
 *
 */
public class FeoZotDanTuoValidator extends FeoValidator {

	@Override
	public int validator(String numbers) throws CoreException {
		if(!numbers.matches("^\\d{1}(,\\d{1}){0,1}\\$\\d{1}(,\\d{1}){1,6}$"))
		{
		...
		int count = MathUtil.getC(secondIntArray.length, 3 - firstIntArray.length)*3;
		if(count < 2)
		{
			throw new CoreException(ErrCode.E2033, ErrCode.codeToMsg(ErrCode.E2033));
		}
