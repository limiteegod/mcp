 ...
 *
 */
public class FeoZtfDanTuoValidator extends FeoValidator {

	@Override
	public int validator(String numbers) throws CoreException {
		if(!numbers.matches("^\\d{1}(,\\d{1}){0,2}\\$\\d{1}(,\\d{1}){1,6}$"))
		{
		...
		//计算注数
		int count = MathUtil.getC(secondIntArray.length, 4 - firstIntArray.length);
		if(count < 2)
		{
			throw new CoreException(ErrCode.E2033, ErrCode.codeToMsg(ErrCode.E2033));
		}
